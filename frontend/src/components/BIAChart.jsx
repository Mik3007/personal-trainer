...
const BIAChart = ({ data }) => {
  ...
  const getComparison = (key) => {
    ...
    const diff = (previous - latest).toFixed(2); // Invertiamo qui per riflettere la perdita come negativa
    const sign = diff > 0 ? '+' : ''; // Segno positivo per incrementi
    return ` (${sign}${diff})`; // Mostra sempre il segno corretto
  };
  
  const getTotalComparison = (key) => {
    if (chartData.length < 2) return '';
    const first = chartData[chartData.length - 1][key]; // Prima misurazione
    const last = chartData[0][key]; // Ultima misurazione
    const totalDiff = (first - last).toFixed(2); // Invertiamo anche qui per riflettere la perdita
    const sign = totalDiff > 0 ? '+' : ''; // Segno positivo per incrementi
    return ` (${sign}${totalDiff})`; // Mostra sempre il segno corretto
  };
  ...
  const renderLegend = (props) => {
    ...
    return (
      <div className="bg-gray-900 p-4 rounded-b-lg mt-4"> {/* Aggiunto margine sopra */}
        {/* Prima riga: Comparazione con la precedente */}
        ...
        {/* Seconda riga: Comparazione con la prima */}
        <h4 className="text-white mb-2">Comparazione con la BIA iniziale :</h4>
        <ul className="flex flex-wrap justify-center gap-4 text-sm mb-8"> {/* Aggiunto mb-8 per più spazio sotto */}
          {payload.map((entry, index) => (
            <li key={`item-${index}`} className="flex items-center">
              <span style={{ backgroundColor: entry.color, width: '10px', height: '10px', display: 'inline-block', marginRight: '5px' }}></span>
  ...
  return (
    <div className="w-full bg-gray-800 rounded-lg overflow-hidden">
      ...
      {/* Legenda posizionata manualmente sotto il grafico */}
