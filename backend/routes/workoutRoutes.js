...
router.get("/:userId", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const workout = await Workout.findOne({ userId: req.params.userId }).sort({
      createdAt: -1,
    });
    if (!workout) {
...
// Rotta per ottenere tutte le schede di un utente
router.get('/:userId/all', ensureAuthenticated, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Errore nel recupero delle schede' });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: "Errore durante l'eliminazione della scheda",
          error: error.message,
        });
    }
  }
